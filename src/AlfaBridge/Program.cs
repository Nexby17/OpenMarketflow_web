        ...
        static void HandleHttp(HttpListenerContext ctx)
        {
            ...
            try
            {
                switch (path)
                {
                    ...
                    case "":
                    case "/index.html":
                        var htmlPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ui.html");
                        if (!File.Exists(htmlPath))
                            htmlPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", "..", "..", "ui.html");
                        if (File.Exists(htmlPath))
                        {
                        ...
                        result = new { error = "ui.html not found", searchedPath = htmlPath };
                        break;
