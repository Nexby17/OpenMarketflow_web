/**
Арбитраж акция/фьючерс — TATN (Татнефть)
Оптимальные параметры: w=15, ez=1.0, xz=0.5
Результат: +25.8%/год | Sharpe 6.77 | WR 96% | 79 сделок
Лот фьючерса = 100 акций
Особенность: xz=0.5 (ждём отскок дальше среднего)

Algorithm = АРБИТРАЖ;
Hash code TATN_ARB_V1_2026
**/

function Initialize()
{
    StrategyName = "Arb_TATN";
    ...
    AddInput("Spot", Inputs.Candle, 1440, true, "TATN=МБ ЦК");
    AddInput("Fut", Inputs.Candle, 1440, true, "TATNфьюч=МБ ФО");
    LongLimit = 0;
    ShortLimit = 0;
}
