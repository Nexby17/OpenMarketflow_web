/**
Арбитраж акция/фьючерс — SBER (Сбербанк)
Оптимальные параметры: w=15, ez=1.5, xz=-0.5
Результат: +18.3%/год | Sharpe 5.00 | WR 96% | 48 сделок
Лот фьючерса = 100 акций
Особенность: xz=-0.5 (выходим немного раньше среднего)

Algorithm = АРБИТРАЖ;
Hash code SBER_ARB_V1_2026
**/

function Initialize()
{
    StrategyName = "Arb_SBER";
    ...
    AddInput("Spot", Inputs.Candle, 1440, true, "SBER=МБ ЦК");
    AddInput("Fut", Inputs.Candle, 1440, true, "SBERфьюч=МБ ФО");
    LongLimit = 0;
    ShortLimit = 0;
}
