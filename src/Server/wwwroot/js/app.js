...
async function renderRobots() {
    ...
    // Order Flow MX Robot row (MXU6, port 5081)
    let ofMxRobotRow = '';
    if (ofMxRobot) {
        const s = ofMxRobot;
        const mode = s.mode || 'stopped';
        ...
        const pnl = s.realizedPnL || 0;
        const paper = s.paper ? ' <span class="badge" style="background:#ff9800">PAPER</span>' : '';
        ofMxRobotRow = `<tr title="Order Flow MXU6 (порт 5081)">
            <td><strong>MXU6</strong></td>
            <td><strong>Order Flow MX</strong> <span class="badge" style="background:#9C27B0">PYTHON</span>${paper}</td>
            <td>${accountDropdownHtml('1225953', '')}</td>
            <td class="${dirCls}">${dirText}${s.avgPrice > 0 ? ' @ ' + s.avgPrice.toFixed(0) : ''}</td>
            <td>—</td>
            <td class="${pnlCls(pnl)}">${pnl >= 0 ? '+' : ''}${pnl.toFixed(0)} ₽</td>
            <td>${s.totalLots || 0}</td>
        ...
        ofMxRobotRow = `<tr title="Order Flow MXU6 (порт 5081)">
            <td><strong>MXU6</strong></td>
            <td><strong>Order Flow MX</strong> <span class="badge" style="background:#9C27B0">PYTHON</span></td>
            <td>${accountDropdownHtml('1225953', '')}</td>
            <td>—</td>
            <td>—</td>
            <td>—</td>
...
// === ORDER FLOW MX ROBOT (порт 5081, MXU6) ===
