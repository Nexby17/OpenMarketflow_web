...
/// 2. RV >= HV → HIGH VOL → закрываем все позиции, не торгуем, ждём
/// 3. SAR×EMA кросс → вход 1 лот + расставляем сетку лимиток против позиции
/// 4. Каждые GridStep пт против позиции → +1 лот (grid fill)
/// 5. Каждый уровень: TP = GridSpread пт от цены заполнения → round trip
/// 6. Закрытие всего: обратный SAR×EMA кросс ИЛИ 50%+ RT + PnL/лот ≥ MinProfitPerLot
/// 7. Dynamic lots 1→3: +1000 руб → +1 лот, убыток → -1 лот
/// </summary>
public class PsarGridStrategy : IStrategy
{
    public class Config
    {
        // SAR
        ...
        public double ClosePct { get; set; } = 0.50;
    ...
    public string Name => "PSAR Grid MM";
    ...
    /// <summary>RV < HV → true (торгуем)</summary>
    public bool IsLowVol => _currentHv > 0 && _currentRv < _currentHv;
    ...
    private const int MAX_IND_HISTORY = 500;

    private readonly List<string> _log = new();
    public IReadOnlyList<string> Log => _log;

    public PsarGridStrategy(Config? config = null)
    {
    ...
    public Signal? OnCandle(Candle candle, string ticker)
    {
        ...
        if (_indicatorHistory.Count > MAX_IND_HISTORY)
            _indicatorHistory.RemoveAt(0);

        bool lowVol = IsLowVol;
        ...
        if (_posDir != 0 && lowVol)
        {
            ...
            // Close conditions
            bool reverseCross = (_posDir == 1 && shortEntry) || (_posDir == -1 && longEntry);
            if (reverseCross)
                return CloseAll(c, ticker, "SAR×EMA reverse");

            if (_peakLots > 1 && _sessionRt >= (int)(_peakLots * Params.ClosePct) && OpenGridLevels > 0)
            {
                if (CurrentGridPnL / OpenGridLevels >= Params.MinProfitPerLot)
                    return CloseAll(c, ticker, $"Profit ({_sessionRt} RT)");
            }
    ...
    private void OpenPosition(int dir, double price, DateTime time, string ticker)
    {
        ...
        {
            double gp = dir == 1 ? price - Params.GridStep * (j + 1) : price + Params.GridStep * (j + 1);
            _grid[j] = new GridLevel
            {
                Price = gp,
                TpPrice = dir == 1 ? gp + Params.GridSpread : gp - Params.GridSpread,
                Filled = false, FillBar = 0
            };
        }

        LogMsg($"[{ticker}] OPEN {dir} @ {price:F0}, lots={_currentLotLevel}, RV={_currentRv:F4} HV={_currentHv:F4}");
    ...
    private void ProcessGrid(double c, double hi, double lo, DateTime time, string ticker)
    {
        ...
        // 1. TP на заполненных уровнях
        if (_entryFilled)
        {
            bool tp = d == 1 ? hi >= _entryPrice + Params.GridSpread : lo <= _entryPrice - Params.GridSpread;
            if (tp)
            {
                _entryFilled = false;
        ...
        for (int j = 0; j < Params.MaxGridLevels; j++)
        {
            if (!_grid[j].Filled || _grid[j].FillBar == bar) continue;
            bool tp = d == 1 ? hi >= _grid[j].TpPrice : lo <= _grid[j].TpPrice;
            if (tp)
            {
        ...
        for (int j = 0; j < Params.MaxGridLevels; j++)
        {
            ...
            bool fill = d == 1 ? lo <= _grid[j].Price : hi >= _grid[j].Price;
            if (fill)
            {
                _grid[j].Filled = true;
                _grid[j].FillBar = bar;
        ...
        OpenGridLevels = _currentLotLevel * (1 + filled);
    ...
    private Signal CloseAll(double price, string ticker, string reason)
    {
        ...
        int d = _posDir;
        double ur = 0; int totalLots = 0;

        if (_entryFilled) { ur += d == 1 ? (price - _entryPrice) : (_entryPrice - price); totalLots += _currentLotLevel; }
        for (int j = 0; j < Params.MaxGridLevels; j++)
            if (_grid[j].Filled) { ur += d == 1 ? (price - _grid[j].Price) : (_grid[j].Price - price); totalLots += _currentLotLevel; }

        double pnl = _sessionRealized + ur * _currentLotLevel - totalLots * Params.Commission;
        LogMsg($"[{ticker}] CLOSE: {reason} | PnL={pnl:F0} | lots={totalLots} | RT={_sessionRt}");
        TotalTrades++;
        TotalPnL += pnl;

        if (pnl < 0) { _currentLotLevel = Math.Max(1, _currentLotLevel - 1); _cumProfit = 0; }
        else { _cumProfit += pnl; if (_cumProfit >= Params.LotStepProfit && _currentLotLevel < Params.MaxLots) { _currentLotLevel++; _cumProfit = 0; } }
    ...
    private void UpdateRvHv(double c)
    {
        ...
        if (_totalBars >= Params.RvWindow + 1)
        {
            ...
            double variance = 0;
            for (int i = 0; i < Params.RvWindow; i++) { double dv = _returns[i] - mean; variance += dv * dv; }
            ...
            {
                int oldest = _rvIdx % Params.HvWindow;
                _rvSum -= _rvValues[oldest];
                _rvValues[oldest] = _currentRv;
                _rvSum += _currentRv;
                _rvIdx++;
            }
