...
/// <summary>
/// Grid MM v7 — чистая стратегия (без исполнения).
/// 
/// Логика:
/// 1. SAR > EMA → SHORT, SAR < EMA → LONG (mean-reversion)
/// 2. Grid работает ВСЕГДА — MaxLots = размер входа, grid уровни сверх него
/// 3. Выход: STD от SAR ИЛИ profit target (RT >= peak*closePct + PnL/lot >= minProfit)
/// </summary>
public class GridMmV7Strategy
{
    public class Config
    {
        ...
        public bool InvertedLogic { get; set; } = false;  // V8: true = SAR>EMA→LONG (trend-following)
    }
    ...
    public GridMmV7Strategy(Config? config = null) { Params = config ?? new Config(); }
    ...
    public void OnCandle(Candle candle)
    {
        ...
        // SAR > EMA → SHORT (mean-reversion), SAR < EMA → LONG
        // V7: SAR>EMA→SHORT (mean-reversion), V8: SAR>EMA→LONG (trend-following)
        bool longSignal, shortSignal;
        if (Params.InvertedLogic)
        {
            longSignal = _warmedUp && prevSarBelow && nowSarAbove;   // V8: SAR↑ → LONG
            shortSignal = _warmedUp && prevSarAbove && nowSarBelow;  // V8: SAR↓ → SHORT
        }
        else
        {
            longSignal = _warmedUp && prevSarAbove && nowSarBelow;   // V7: SAR↓ → LONG
            shortSignal = _warmedUp && prevSarBelow && nowSarAbove;  // V7: SAR↑ → SHORT
        }
        
        if (longSignal || shortSignal)
            Console.WriteLine($"[V7-SIGNAL] {(longSignal ? "LONG" : "SHORT")} cross: prevSAR={_prevSar:F0} prevEMA={_prevEma:F0} → SAR={sar:F0} EMA={ema:F0} price={candle.Close:F0}");
        ...
        // 0. Reverse signal — закрыть если PnL > 0
        if ((_posDir == 1 && shortSignal) || (_posDir == -1 && longSignal))
        {
            double sessionPnL = CalcUnrealizedPnL(candle.Close);
            if (sessionPnL > 0)
            {
                EmitCloseAll($"Reverse signal: PnL={sessionPnL:F0}");
                return;
            }
        }
        
        // 1. STD exit — только если PnL > 0
    ...
    /// <summary>TP fill закрывает 1 лот — уменьшаем filled levels</summary>
    public void OnTpFilled()
    {
        if (_filledLevels > 0) _filledLevels--;
    }
    
    /// <summary>Сбросить grid на уровень 1 после TP fill (мельница)</summary>
    public void ResetGridToLevel1()
    {
        _currentLevel = 1;
        _filledLevels = Math.Max(0, _filledLevels - 1);
    }
    ...
    public void ForceEntry(double currentPrice)
    {
        ...
        int dir = Params.InvertedLogic
            ? (diff >= -5 ? 1 : -1)   // V8: SAR > EMA → LONG
            : (diff >= -5 ? -1 : 1);  // V7: SAR > EMA → SHORT
        EmitEntry(dir, currentPrice, $"Force: SAR={CurrentSar:F0} EMA={CurrentEma:F0} diff={diff:F0} → {(dir == 1 ? "LONG" : "SHORT")}");
    }
    ...
    public string GetStatus()
    {
        ...
        string hold = Params.GridHold ? "Hold" : "STD";
        return $"Mode={CurrentMode} | {dir} entry={_entryPrice:F0} lots={TotalLots} " +
               $"filled={_filledLevels} RT={_roundTrips} PnL={_realizedPnL:F0} " +
               $"next_level={_currentLevel} | SAR={CurrentSar:F0} EMA={CurrentEma:F0} STD={CurrentStd:F0} " +
               $"| {hold}(m={Params.StdMult})";
    }
