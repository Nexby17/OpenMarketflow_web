...
class ArbOrderManager:
    ...
    def __init__(self, dp_url: str = "http://localhost:5060", account: str = ""):
        ...
        self._stock_account = os.environ.get("FINAM_ACCOUNT", account)  # same account for stocks
        self._timeout = 5.0
        ...
        self._on_unauthenticated = None  # callback for JWT reconnect

        # MOEX stocks: gRPC requires quantity in SHARES, not lots.
        # Lot sizes for common MICEX stocks (default 10).
        ...
        self._order_symbol_map: dict[str, str] = {}  # order_id → symbol (for lot conversion)
    ...
    def _shares_to_lots(self, order_id: str, filled_qty: int) -> int:
        """Convert shares back to lots for stock orders. No-op for futures."""
        sym = self._order_symbol_map.get(order_id, "")
        if not sym:
            return filled_qty
        base = sym.split('@')[0]
        lot_size = self._lot_sizes.get(base, self._default_lot_size)
        if lot_size > 1 and filled_qty % lot_size == 0:
            return filled_qty // lot_size
        return filled_qty

    def cancel_pending_orders(self, symbol_a: str, symbol_b: str):
        """Cancel all open/pending orders for both symbols on startup.
        Prevents phantom fills from previous sessions."""
            ...
            for acc in [self._stock_account, self._account]:
                try:
                    resp, _ = self._fp.orders_stub.GetOrders.with_call(
                        request=OrdersRequest(account_id=acc),
                        timeout=5, metadata=(self._fp.metadata,))
                    if hasattr(resp, 'orders'):
                        for o in resp.orders:
                            state = int(o.status) if hasattr(o, 'status') else 0
                            oid = o.order_id if hasattr(o, 'order_id') else ''
                            # status 1=NEW, 2=PARTIALLY_FILLED, 3=FILLED, 5=CANCELED
                            if state in (1, 2) and oid:
                                log.warning(f"Cancelling old pending order {oid} on acc={acc}")
                                self._fp.orders_stub.CancelOrder.with_call(
                                    request=CancelOrderRequest(account_id=acc, order_id=oid),
                                    timeout=5, metadata=(self._fp.metadata,))
                except Exception as e:
                    log.debug(f"Cancel pending orders acc={acc}: {str(e)[:60]}")
        except Exception as e:
            log.error(f"Cancel pending orders error: {e}")
    ...
    def _grpc_place_order(self, symbol: str, side: str, quantity: int,
        ...
        try:
            ...
            if is_stock:
                ...
                # Finam gRPC: quantity is already in shares (caller must convert lots->shares)
                grpc_qty = quantity
            else:
                acc = self._account
            ...
            self._order_symbol_map[oid] = order_symbol
            ...
            # Trigger reconnect callback if JWT expired
            if hasattr(e, 'code') and e.code().name == 'UNAUTHENTICATED' and self._on_unauthenticated:
                log.warning("gRPC order UNAUTHENTICATED — triggering reconnect callback")
                try:
                    self._on_unauthenticated()
                except Exception as cb_err:
                    log.error(f"Reconnect callback error: {cb_err}")
            return None
    ...
    def execute_entry(self, symbol_a: str, symbol_b: str,
        ...
        # --- REAL MODE ---
        # Leg A: LIMIT (for stocks, convert lots to shares)
        qty_a = lots_a * self._lot_sizes.get(symbol_a.split('@')[0], self._default_lot_size) if self._is_stock(symbol_a) else lots_a
        qty_b = lots_b  # futures already in contracts
        oid_a = self._place_limit(symbol_a, side_a, qty_a, limit_price_a, tag="arb_entry_a")
        if not oid_a:
        ...
        # Check min fill ratio (qty_a is in shares)
        # If partial fill < min_fill_ratio — TOP UP with market order (never abort arbitrage entry)
        remaining_qty = qty_a - leg_a.quantity
        if remaining_qty > 0:
            log.warning(f"Leg A partial fill: {leg_a.quantity}/{qty_a} — topping up {remaining_qty} via market")
            topup = self._place_market(symbol_a, side_a, remaining_qty, tag="arb_entry_a_topup")
            if topup and topup.get("order_id"):
                topup_oid = topup["order_id"]
                # Wait briefly for market topup fill
                topup_price = self._get_real_fill_price(topup_oid, limit_price_a)
                if topup_price > 0:
                    # Weighted average price
                    leg_a = LegResult(filled=True,
                                       price=(leg_a.price * leg_a.quantity + topup_price * remaining_qty) / qty_a,
                                       quantity=qty_a, order_id=leg_a.order_id)
                    log.info(f"Leg A topped up: total {qty_a}@{leg_a.price:.2f}")
                else:
                    # Assume filled at limit price
                    leg_a = LegResult(filled=True, price=leg_a.price,
                                       quantity=qty_a, order_id=leg_a.order_id)
            else:
                log.error(f"Leg A topup market FAILED for {remaining_qty} {symbol_a} — proceeding with partial {leg_a.quantity}")
        ...
        # Convert leg_a filled shares back to lots for caller
        shares_a = leg_a.quantity
        if self._is_stock(symbol_a) and shares_a > 0:
            ls = self._lot_sizes.get(symbol_a.split('@')[0], self._default_lot_size)
            if ls > 1:
                lots_a_filled = shares_a // ls
                log.info(f"Leg A filled: {shares_a} shares = {lots_a_filled} lots")
                result["leg_a"] = LegResult(filled=True, price=leg_a.price,
                                            quantity=lots_a_filled, order_id=leg_a.order_id)
        result["success"] = True
        return result
