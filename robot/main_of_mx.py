...
# PORT-A1: bootstrap paths + .env (py4 SDK 4.3.3, robot/, project root with finam_hub.py)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        DailyFileHandler(LOG_DIR, 'of_mx'),
    ],
)
...
def connect_finam():
    ...
    # PORT-B1 preview: REST4 probe (non-fatal)
    try:
...
def _reconnect_finampy():
    """Watchdog: reconnect hub subscriptions (PORT-A1 lab pattern)."""
...
def main_loop():
    ...
    while _running:
        try:
            ...
            # PORT-A1: fill-channel = WS ORDERS (real-mode, post-F decision);
            # paper-mode fills are simulated by OrderManagerRest4 (PORT-B1)
            pass
...
def _get_broker_position_fast():
    """Broker |lots| via REST 4.3.3; None = позиция НЕИЗВЕСТНА (API failed) (PORT-B1)."""
    try:
    ...
    if act == "close_all":
        ...
        if result:
            ...
            if broker_lots is None:
                ...
                strategy._lot_queue.append(LotEntry(price=action.get('entryPrice', strategy._current_price), side=action.get('entrySide', strategy._dir), lots=qty))
            ...
            if length > 0:
                try:
                    ...
                    if lots == 0 and price > 0:
                        self._json(400, {"error": "lots=0 игнорирует цену — укажите лоты"})
                        return
                    if lots != 0 and price <= 0:
                    ...
                    # Применяем: абсолютная позиция {lots: +N лонг / -N шорт / 0 флэт}
